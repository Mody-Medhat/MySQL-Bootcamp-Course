...
-- ------------------------------------------------------------------------------------ --
	...
	CREATE TABLE reviews (
		...
		CONSTRAINT reviews_reviewers_FK FOREIGN KEY (series_id)
			REFERENCES series (id)
			ON DELETE CASCADE,
		CONSTRAINT reviews_series_FK FOREIGN KEY (reviewer_id)
    ...
    -- 6 Challenge
		...
		SELECT 
			first_name,
			last_name,
			ROUND(IFNULL(AVG(rating), 0), 0) AS 'AVG',
		...
		FROM
			reviewers Rs
				LEFT JOIN
			reviews R ON Rs.id = R.series_id
		GROUP BY 1 , 2;
